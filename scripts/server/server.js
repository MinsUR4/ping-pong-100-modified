(function() {  
  ...
  const WebSocketServer = require('ws').Server;a(function() {  
    'use strict';
    ...
    // Assign paddle spawn side dynamically
    const getX = (paddle) => {
      let leftCount = 0;
      let rightCount = 0;
  
      for (const client of wss.clients) {
        if (!client.paddle || !client.paddle.position?.x) continue;
      ...
      return leftCount > rightCount
        ? utils.randomIntBetween(50, 100 - paddle.width)  // Spawn on right
        : utils.randomIntBetween(0, 50 - paddle.width);   // Spawn on left
    };
    ...
    wss.on('connection', (ws) => {
      ...
      // Notify existing players of the new connection
    ...
    setInterval(() => {
      ...
      for (const client of wss.clients) {
        if (hasBounced || !client.paddle) continue;
        ...
        if (isColliding(client.paddle, ball)) {
